...
function run(cmd: string, cwd: string = '') {
  const result = spawnSync(cmd, { shell: true, stdio: 'inherit', cwd: path.join(__dirname, '..', cwd || '') });
...
if (nodeMajorVersion <= 10) {
  ...
  if (nodeMajorVersion === 8) {
    ...
    // We need to cast the  new `astTransformers` value to `any` because we're now violating the new schema, on which
    // our automatically-generated types file is based.
    ...
    code = code.replace(`"${cwd}"`, 'process.cwd()');
    ...
    // small subset we're about to install below. Since they're browser-based, these tests are never going to be running
    // in a node 8 environment in any case, so it's fine to skip them here. (In the long run, we should only run such
    // tests against a single version of node, but in the short run, this at least allows us to not be blocked by the
    // failures.)
